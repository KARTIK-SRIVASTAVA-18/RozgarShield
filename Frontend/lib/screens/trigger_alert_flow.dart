...
class _TriggerAlertFlowState extends State<TriggerAlertFlow>
  ...
  List<Map<String, dynamic>> get _checks {
    ...
    return [
      ...
      {
        'label': 'Policy active & eligible',
        'icon': Icons.shield_rounded,
        'color': _isFraud ? const Color(0xFFFF5252) : const Color(0xFFF5A623),
        'isFraud': _isFraud,
      },
      {
        'label': _isFraud
  ...
  String _getEmojiForTrigger(String type, String name) {
    final t = type.toLowerCase();
    final n = name.toLowerCase();
    if (t.contains('rain') || n.contains('rain')) return '🌧️';
    if (t.contains('flood') || n.contains('flood')) return '🌊';
    if (t.contains('heat') || n.contains('heat')) return '🌡️';
    if (t.contains('aqi') || n.contains('aqi')) return '😷';
    if (t.contains('cyclone') || n.contains('cyclone')) return '🌀';
    if (t.contains('curfew') || n.contains('curfew') || t.contains('shutdown')) return '🛑';
    if (t.contains('landslide') || n.contains('landslide')) return '⛰️';
    return '⚠️';
  }
  ...
  void initState() {
    ...
    _triggerIcon = widget.triggers.map((t) {
      final type = t['trigger_type']?.toString() ?? t['type']?.toString() ?? '';
      final name = t['name']?.toString() ?? t['label']?.toString() ?? '';
      return _getEmojiForTrigger(type, name);
    }).join(' ');

    if (_triggerIcon.trim().isEmpty) {
      _triggerIcon = '⚠️';
    }
  ...
  Future<void> _downloadReceipt() async {
    ...
    try {
      await ClaimReceiptGenerator.generate(
        ...
        amounts:  widget.triggers.map((t) => t['amount'] as int? ?? 0).toList(),
        policy:   widget.policy,
        upiId:    '${(widget.policy?['name'] ?? 'worker').toString().replaceAll(' ', '').toLowerCase()}${(widget.policy?['phone'] ?? '').toString().replaceAll(' ', '')}@upi',
        claimId:  _claimId,
        txnId:    _txnId,
        isFraud:  _isFraud,
      );
    ...
    child: Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(children: [
            Text(_triggerIcon, style: const TextStyle(fontSize: 32)),
  ...
  Widget _payoutScreen() {
    final upi = '${(widget.policy?['name'] ?? 'worker').toString().replaceAll(' ', '').toLowerCase()}${(widget.policy?['phone'] ?? '').toString().replaceAll(' ', '')}@upi';
      ...
      child: Column(children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _downloading ? Colors.white.withOpacity(0.02) : Colors.white.withOpacity(0.05),
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
                color: Colors.white.withOpacity(0.1))),
